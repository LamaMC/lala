...
const FARM_ACCOUNT   = { username: 'JamaaLcaliph', loginCommand: '/login 3043AA' };
  ...
  try {
    ...
    function openTeleportGUI () {
      ...
      const fallback = setTimeout(() => {
        ...
        console.log('⚠️ [JamaaLcaliph] windowOpen timed out — disconnecting to reconnect and start over.');
        alive = false;
        bot.quit();
      }, 6000);
    ...
    bot.on('login', () => console.log('🔌 [JamaaLcaliph] Login packet sent.'));
    bot._client.on('error', err => console.log('🔥 [JamaaLcaliph] Client error:', err.message));

    bot.once('spawn', () => {
      console.log('🟢 [JamaaLcaliph] SPAWN EVENT FIRED');
      try {
    ...
    bot.on('end', (reason) => {
      console.log('📋 [JamaaLcaliph] End reason:', reason);
      alive = false;
      stopFarming();
      if (bot.pingShutdown) {
      ...
      if (bot.manualQuit) {
        console.log('🛑 Manual quit (regrow handoff) — not reconnecting as JamaaLcaliph.');
        return;
      }
      if (scriptEnabled) {
        console.log('🔁 Disconnected unexpectedly. Reconnecting as JamaaLcaliph in 5s...');
        setTimeout(createFarmBot, 5000);
      }
    });

    bot.on('error', err => console.log('❌ [JamaaLcaliph] Error:', err.message));
  ...
  try {
    ...
    function openTeleportGUI () {
      ...
      const fallback = setTimeout(() => {
        ...
        handled = true;
        bot.removeListener('windowOpen', onWindow);
        console.log('⚠️ [Beastro] windowOpen timed out — disconnecting to reconnect and start over.');
        alive = false;
        bot.quit();
      }, 6000);
