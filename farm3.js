...
const FARM_ACCOUNT   = { username: 'LamaMC', loginCommand: '/login 3195' };
...
function createFarmBot () {
  ...
  try {
    ...
    bot.on('end', (reason) => {
      ...
      if (bot.manualQuit) {
        console.log('🛑 Manual quit (regrow handoff) — not reconnecting as LamaMC.');
        return;
      }
      if (scriptEnabled) {
        console.log('🔁 Disconnected unexpectedly. Reconnecting as LamaMC in 5s...');
