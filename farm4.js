  ...
  try {
    ...
    function openTeleportGUI () {
      ...
      const fallback = setTimeout(() => {
        ...
        console.log('⚠️ [Banarsi] windowOpen timed out — disconnecting to reconnect and start over.');
        alive = false;
        bot.quit();
      }, 6000);
    ...
    bot.on('login', () => console.log('🔌 [Banarsi] Login packet sent.'));
    bot._client.on('error', err => console.log('🔥 [Banarsi] Client error:', err.message));

    bot.once('spawn', () => {
      console.log('🟢 [Banarsi] SPAWN EVENT FIRED');
      try {
    ...
    bot.on('end', (reason) => {
      console.log('📋 [Banarsi] End reason:', reason);
      alive = false;
      stopFarming();
      if (bot.pingShutdown) {
      ...
      if (bot.manualQuit) {
        console.log('🛑 Manual quit (regrow handoff) — not reconnecting as Banarsi.');
        return;
      }
      if (scriptEnabled) {
        console.log('🔁 Disconnected unexpectedly. Reconnecting as Banarsi in 5s...');
        setTimeout(createFarmBot, 5000);
      }
    });

    bot.on('error', err => console.log('❌ [Banarsi] Error:', err.message));
  ...
  try {
    ...
    function openTeleportGUI () {
      ...
      const fallback = setTimeout(() => {
        ...
        handled = true;
        bot.removeListener('windowOpen', onWindow);
        console.log('⚠️ [Enrage] windowOpen timed out — disconnecting to reconnect and start over.');
        alive = false;
        bot.quit();
      }, 6000);
      ...
      async function onWindow (window) {
        ...
        if (slot && slot.name !== 'air') {
          try {
            await bot.clickWindow(20, 0, 1);
            console.log('🎯 [Enrage] Clicked teleport item.');
          } catch (err) {
            console.log('❌ [Enrage] GUI click error:', err.message);
          }
        }
        if (!alive) return;
        setTimeout(() => {
    ...
    function enterAfkPool () {
      console.log(`⏳ [Enrage] AFK regrow wait started (${REGROW_DURATION_MS / 1000}s).`);
      regrowTimer = setTimeout(() => {
        if (!alive) return;
        console.log(`✅ [Enrage] Regrow wait done — handing back to ${FARM_ACCOUNT.username}.`);
        alive = false;
        bot.manualQuit = true;
        bot.quit();
    ...
    function handlePing () {
      if (pingPaused || !alive) return;
      pingPaused = true;
      console.log('🔔 [Enrage] Ping detected — going fully AFK for 5 min, then disconnecting.');
      if (regrowTimer) clearTimeout(regrowTimer);
      setTimeout(() => {
        if (!alive) return;
        console.log('🛑 [Enrage] 5 min AFK done — disconnecting. Re-run the script to resume.');
        scriptEnabled    = false;
        bot.pingShutdown = true;
        alive            = false;
        bot.quit();
    ...
    bot.on('login', () => console.log('🔌 [Enrage] Login packet sent.'));
    bot._client.on('error', err => console.log('🔥 [Enrage] Client error:', err.message));

    bot.once('spawn', () => {
      console.log('🟢 [Enrage] SPAWN EVENT FIRED');
      try {
        bot._client.socket.setTimeout(24 * 60 * 60 * 1000);
        bot._client.socket.setKeepAlive(true, 10000);
      } catch (e) { console.log('⚠️ [Enrage] socket setup failed:', e.message); }
      console.log('✅ [Enrage] Spawned');
      bot.manualQuit = false;
      setTimeout(() => {
        if (!alive) return;
    ...
    bot.on('death', () => {
      if (!alive) return;
      console.log('☠️ [Enrage] Died — re-warping and resuming wait.');
      if (regrowTimer) clearTimeout(regrowTimer);
      setTimeout(() => {
        if (!alive) return;
        bot.chat(WARP_COMMAND);
    ...
    bot.on('end', (reason) => {
      console.log('📋 [Enrage] End reason:', reason);
      alive = false;
      if (regrowTimer) clearTimeout(regrowTimer);

      if (bot.pingShutdown) {
        console.log('🛑 [Enrage] Stopped after a ping — script paused. Re-run to resume.');
        return;
      }
      if (bot.manualQuit) {
        console.log('🛑 [Enrage] Manual quit — not reconnecting as Enrage.');
        return;
      }
      if (scriptEnabled) {
        console.log('🔁 [Enrage] Disconnected unexpectedly. Reconnecting in 5s...');
        setTimeout(createRegrowBot, 5000);
      }
    });

    bot.on('error', err => console.log('❌ [Enrage] Error:', err.message));
